...
//* str means string. 'string' word is a javascript word, better to use str
...
const firstLetters = (arr) => {
  // Solution code here...
  return arr.map(index => index.charAt(0));
};
...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter(index => index.includes(':)'));
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(index => `${index.substring(1, 4)}${index.substring(6, 9)}${index.substring(10, 14)}`);
};
